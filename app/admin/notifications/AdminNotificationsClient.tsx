...
export default function AdminNotificationsClient() {
  ...
  const loadPage = useCallback(async (nextPage: number, append: boolean) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/notifications?page=${nextPage}&limit=10`, {
        cache: "no-store",
      });
      if (!res.ok) {
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-2 font-montserrat text-sm text-[#5E5E5E]">
            Doctor registrations, approvals, and platform alerts.
          </p>
        </div>
      </div>
      ...
      {error ? (
        ...
        <div className="mt-8 rounded-xl border border-dashed border-[#e5e5e5] bg-[#fcfcfc] p-8 text-center">
          ...
          <p className="mt-1 font-montserrat text-sm text-[#5E5E5E]">
            Important admin updates will appear here.
          </p>
        </div>
      ) : (
